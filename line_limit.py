...
if 'Jacobian' in file_name or 'Linear' in file_name: 
    ...
    output = list(open('model_Rates.f90'))
    for line in xrange(len(output)):
        if 'RO2 =' in output[line]: 
            output[line] = checker(output[line])
    ...
    for line in xrange(len(output)):
        current = output[line]
        if 'RO2 =' in current and 'IGNORE' not in current:strip = True
        if strip:
            
            if 'CALL' in current: 
                strip=False
                print len(checker(string)), 'dssdaf',len(string),pd.Series(checker(string))
                newarr.append(checker(string))
                newarr.append('\n'+current)

            else: 
                string += (current.replace('&','').replace('\n',''))
