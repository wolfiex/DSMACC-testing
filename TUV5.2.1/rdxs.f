...
* also values of significant wavelengths, converted to vacuum
      ...
      REAL iup273(kw), iup203(kw)
      REAL v238, v347, v362, v410
...
* bas = Bass et al.
* iup = IUPAC recommadation (last data sheet update 2001)
      ...
      CALL o3_iup(nw,wl, iup203,iup273, v238,v347,v362,v410,v195,v345)
...
* note limited wavelength range 245-342
      ...
      ELSEIF(mabs .EQ. 5 .OR. mabs .EQ. 6) THEN

* use IUPAC recomandation
* 5) with interpolation between 362.5 and 410nm
* 6) with cross sections forced to 0 between 362.5 and 410nm
* (with last data sheet update from 2001):

         DO iw = 1, nw-1
          DO iz = 1, nz

            xs(iz,iw) = iup203(iw) +
     $           (iup273(iw) - iup203(iw))*(t(iz) - 203.)/(273. - 203.)
            IF (t(iz) .LE. 203.) xs(iz,iw) = iup203(iw)
            IF (t(iz) .GE. 273.) xs(iz,iw) = iup273(iw)
            IF (mabs==6 .AND. wl(iw).GE.v362 .AND. wl(iw).LE.v410)
     &         xs(iz,iw) = 0.
...
* use IUPAC recomandation with high res. data by Malicet et al. (1995)
* between 195 and 345nm and with cross sections forced to 0 between 362.5 and 410nm
* (with last data sheet update from 2001):

         DO iw = 1, nw-1
          DO iz = 1, nz

         IF(wl(iw) .LT. v195) THEN
            xs(iz,iw) = iup273(iw)
         ENDIF

         IF(wl(iw) .GE. v195 .AND. wl(iw) .LE. v345) THEN
            IF (t(iz) .GE. 218. .AND. t(iz) .LT. 228.) THEN
               xs(iz,iw) = rei218(iw) +
     $              (t(iz)-218.)*(rei228(iw)-rei218(iw))/(228.-218.)
            ELSEIF (t(iz) .GE. 228. .AND. t(iz) .LT. 243.) THEN
     ...
     $              (t(iz)-243.)*(rei295(iw)-rei243(iw))/(295.-243.)
         ...
         IF(wl(iw) .GT. v345) THEN
            xs(iz,iw) = iup203(iw) +
     $           (iup273(iw) - iup203(iw))*(t(iz) - 203.)/(273. - 203.)
            IF (t(iz) .LE. 203.) xs(iz,iw) = iup203(iw)
            IF (t(iz) .GE. 273.) xs(iz,iw) = iup273(iw)
            IF (wl(iw).GE.v362 .AND. wl(iw).LE.v410) xs(iz,iw) = 0.
         ENDIF
...
*=============================================================================*
      ...
      SUBROUTINE o3_iup
     &           (nw,wl, iup203,iup273, v238,v347,v362,v410,v195,v345)

*-----------------------------------------------------------------------------*
*=  PURPOSE:                                                                 =*
*=  Read and interpolate the O3 cross section from IUPAC recommendation      =*
*=  (last data sheet update 2001)                                            =*
*-----------------------------------------------------------------------------*
*=  PARAMETERS:                                                              =*
*=  NW     - INTEGER, number of specified intervals + 1 in working        (I)=*
*=           wavelength grid                                                 =*
...
* output
      ...
      REAL iup203(kw), iup273(kw)
      REAL v238, v347, v362, v410, v195, v345

***********

      OPEN(UNIT=kin,FILE='DATAE1/O3/IUPAC_O3.abs',STATUS='old')
      DO i = 1, 5
         read(kin,*)
      ENDDO
      n1 = 32
...
* convert wavelengths to vacuum
      ...
      CALL inter2(nw,wl,iup273,n3,x1,y1,ierr)
      IF (ierr .NE. 0) THEN
         WRITE(*,*) ierr, 'Error. O3 cross section - IUPAC'
         STOP
      ENDIF

      CALL addpnt(x2,y2,kdata,n4,x2(1)*(1.-deltax),0.)
      CALL addpnt(x2,y2,kdata,n4,               0.,0.)
      CALL addpnt(x2,y2,kdata,n4,x2(n4)*(1.+deltax),0.)
      CALL addpnt(x2,y2,kdata,n4,           1.e+38,0.)
      CALL inter2(nw,wl,iup203,n4,x2,y2,ierr)
      IF (ierr .NE. 0) THEN
         WRITE(*,*) ierr, 'Error. O3 cross section - IUPAC'
         STOP
      ENDIF
...
* wavelength breaks must be converted to vacuum:
      ...
      v238 = 238.2 * refrac(238.2, 2.45E19)
      v347 = 347.5 * refrac(347.5, 2.45E19)
      v362 = 362.5 * refrac(362.5, 2.45E19)
      v410 = 410.0 * refrac(410.0, 2.45E19)
      v195 = 195.00 * refrac(195.00, 2.45E19)
      v345 = 345.00 * refrac(345.00, 2.45E19)
...
* 5 = JPL 2006, bin-to-bin interpolation
      ...
      IF(vers==1 .OR. vers==2) THEN
       ...
       ELSEIF(mabs.EQ.1) THEN
        WRITE(kout,'(2A)')
     &       ' NO2 cross section for transmission',
     &       ' from Davidson et al. (1988).'
       ELSEIF(mabs.EQ.2) THEN
        WRITE(kout,'(2A)')
     &       ' NO2 cross section for transmission from JPL',
     &       ' 1994 - 2002 evaluations.'
       ELSEIF(mabs.EQ.3) THEN
        WRITE(kout,'(A)')
     &       ' NO2 cross section for transmission from Harder et al.'
       ELSEIF(mabs.EQ.4) THEN
        WRITE(kout,'(2A)')
     &       ' NO2 cross section for transmission from JPL 2006,',
     ...
     &       ' from IUPAC recommendation.'
       ELSE
        STOP "'mabs' not defined for NO2 transmission."
      ENDIF
      ...
      IF (mabs .EQ. 6) CALL no2xs_iup(nz,tlay,nw,wl, no2xs)
...
*=============================================================================*
      ...
      SUBROUTINE no2xs_iup(nz,t,nw,wl, no2xs)

* read and interpolate NO2 xs from IUPAC recommendation
...
* with interpolation of bin midpoints
      ...
      OPEN(UNIT=kin,FILE='DATAE1/NO2/NO2_IUPAC.abs',STATUS='old')
      DO i = 1, 5
         READ(kin,*)
      ENDDO
      n1 = 59
      ...
      DO iw = 1, nw-1
         DO iz = 1, nz
            no2xs(iz,iw) = yg1(iw) +
     $           (yg2(iw)-yg1(iw))*(t(iz) - 220.)/(298. - 220.)
         ENDDO
      ENDDO
      RETURN
      END
