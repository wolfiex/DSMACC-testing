...
*                                     (MCM standard)
     ...
     &                 ' calculated with data from JPL 2006 evaluation.'
       ELSEIF(mabs.EQ.3) THEN
        WRITE(kout,'(2A)') ' O3 cross section for photolysis values',
     &              ' calculated with data from WMO 1985 O3 assessment.'
...
!                 • HONO + O(1D)
      ...
      IF(vers==1 .OR. vers==2) THEN
       ...
       ELSEIF(myld.EQ.1) THEN
        WRITE(kout,'(A)')
     &       ' HNO2 quantum set to unity.'
       ELSEIF(myld.EQ.2) THEN
        WRITE(kout,'(A)')
     &       ' HNO2 quantum yields from IUPAC recommendation.'
       ELSE
...
* JPL quantum yield data
     ...
     $                 * (300. - tlev(iz))/80.)
               ...
               IF(iz==1.or.iz==40.or.iz==80.or.iz==120)
     &           print*,wc(iw),iz,qym300,akt,qymt,
     &                  qyr300,qyr300+qymt,airden(iz)
...
*     with T/p-dependence: DATAJ1/CH3CHO/CH3CHO_pdep_iup13.yld
     ...
     &       ' CH3CHO cross section from JPL 2011.'
      ...
      IF(vers==1 .OR. vers==2) THEN
       ...
       ELSEIF(myld.EQ.1) THEN
        WRITE(kout,'(A)')
     &       ' CH3CHO quantum yields from IUPAC 97.'
       ELSEIF(myld.EQ.2) THEN
        WRITE(kout,'(A)') ' CH3CHO quantum yields from NCAR.'
       ELSEIF(myld.EQ.3) THEN
        WRITE(kout,'(A)')
     &       ' CH3CHO quantum yields from IUPAC 2013.'
       ELSE
...
* pressure-dependence parameters
     ...
     $     STATUS='old')
         ...
         CALL addpnt(x1,y1,kdata,n1,x1(1)*(1.-deltax),0.)
         CALL addpnt(x1,y1,kdata,n1,               0.,0.)
         CALL addpnt(x1,y1,kdata,n1,x1(n)*(1.+deltax),0.)
         CALL addpnt(x1,y1,kdata,n1,           1.e+38,0.)
         CALL inter2(nw,wl,yg4,n1,x1,y1,ierr)
         IF (ierr .NE. 0) THEN
            WRITE(*,*) ierr, jlabel(j)
...
* or on overall quantum yield based on IUPAC 2013
            IF(mpress==1) THEN
!             qy1 = MAX(0.,MIN(1.,qy1 * (1. + yg4(iw))/(1. + yg4(iw)
!    &              *airden(i)/2.465E19)))
              qy1=MAX(0.,MIN(1.,
...
* 7:  Quantum yields from IUPAC (2003) assumed as air
      ...
      IF(vers==1 .OR. vers==2) THEN
       ...
       ELSEIF(mabs.EQ.1) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCHO cross section from IUPAC 1997.'
       ELSEIF(mabs.EQ.2) THEN
        WRITE(kout,'(2A)')
     ...
     &       ' CH3COCHO quantum yields from IUPAC in N2.'
       ELSEIF(myld.EQ.7) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCHO quantum yields from IUPAC assumed in air.'
       ELSE
...
c               kq = 1.93e4 * EXP(-5639/wc(iw))
            ...
            ELSEIF(myld .GE. 6) THEN
             ...
             IF(myld==6) kq = kq * 0.7808

             qy = 1. / (phi0 + kq * airden(i) * 760./2.456E19)
     ...
     &       ' PAN quantum yields from the book of Calvert et al. 2011.'
      ...
      DO iw = 1, nw-1
        IF (myld == 1) THEN
          ...
          qyNO3 = 0.3
         ELSEIF (myld == 2) THEN
          IF(wl(iw).LE.248.) THEN
            qyNO2 = 0.76
            qyNO3 = 0.24
           ELSEIF(wl(iw).GE.308.) THEN
            qyNO2 = 0.59
            qyNO3 = 0.41
           ELSE
            qyNO2 = 0.76-(wl(iw)-248.)/(308.-248.)*(0.76-0.24)
            qyNO3 = 1 - qyNO2
          ENDIF
         ELSEIF(myld == 3) THEN
          IF(wc(iw)<=278.) THEN
...
* same channels estimated as for acrolein
      ...
      j = j+1
      jlabel(j) = 'CH2=C(CH3)CHO -> CH3C:CH3 + CO'
      j = j+1
...
* or estimates by Calvert et al. (2011)
     ...
     $       STATUS='OLD')
      ...
      DO iw = 1, nw-1
        ...
        DO i = 1, nz
* product distribution as estimated from Calvert et al. 2011 for acrolein:
           qy1 = qy * 0.0018
           qy2 = qy * (-2.826E-23*airden(i)+0.0018)
           qy3 = qy * (0.0195*EXP(-airden(i)/6.8586e18)+0.0028)
           qy4 = qy * (0.0024*EXP(-airden(i)/1.8491e19)+0.0006)
           sq(j-3,i,iw) = qy1 * sig
           sq(j-2,i,iw) = qy2 * sig
           sq(j-1,i,iw) = qy3 * sig
           sq(j  ,i,iw) = qy4 * sig
...
*=============================================================================*
      ...
      SUBROUTINE r106(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! CH3CH2ONO2
...
* combine:
      ...
      DO iw = 1, nw - 1
        ...
        IF(mabs==1)THEN
          sig = sig1
         ELSEIF(mabs==2)THEN
          IF(wc(iw)>=245. .AND. wc(iw)<=340.) THEN
            sig = yg(iw)
           ELSE
            sig = sig1
          ENDIF
         ELSEIF(mabs==3)THEN
          sig = sig2
         ELSEIF(mabs==4)THEN
          IF(wc(iw)>=250. .AND. wc(iw)<=330.) THEN
            sig = yg(iw)
           ELSE
            sig = sig2
          ENDIF
...
*=============================================================================*
      ...
      SUBROUTINE r141(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! C2H5ONO2 + hv
