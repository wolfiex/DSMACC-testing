...
* 6 = IUPAC recommendation (MCM standard)
      ...
      IF(vers==1 .OR. vers==2) THEN
       ...
       ELSEIF(mabs.EQ.1) THEN
        WRITE(kout,'(A)')
     &       ' NO2 cross section from Davidson et al. (1988).'
       ELSEIF(mabs.EQ.2) THEN
        WRITE(kout,'(A)')
     &       ' NO2 cross section from JPL 1994 - 2002 evaluations.'
       ELSEIF(mabs.EQ.3) THEN
        WRITE(kout,'(A)')
     &       ' NO2 cross section from Harder et al.'
       ELSEIF(mabs.EQ.4) THEN
        WRITE(kout,'(2A)')
     &       ' NO2 cross section from JPL 2006,',
     ...
     &       ' bin-to-bin interpolation.'
       ELSEIF(mabs.EQ.6) THEN
        WRITE(kout,'(A)')
     &       ' NO2 cross section from IUPAC recommendation.'
       ELSE
...
* IUPAC recommendation
        ...
        CALL inter2(nw,wl,yg1,n1,x1,y1,ierr)
        IF (ierr .NE. 0) THEN
         WRITE(*,*) ierr, 'Error. O3 cross section - IUPAC'
         STOP
        ENDIF
...
*--- Cross section for cases 1 and 3:
     ...
     &       ' H2O2 quantum yields set to 1 (TUV standard).'
       ELSEIF(myld.EQ.2) THEN
        WRITE(kout,'(A)')
     &    ' H2O2 quantum yields set to 1 for wavelengths greater 280nm'
        WRITE(kout,'(A)')
...
* combine:
      ...
      OPEN(111,FILE='qymcm.dat')
      OPEN(112,FILE='xmcm.dat')
      WRITE(111,
     &     "('lambda qy1(n0)  qy1(1) qy1(40) qy1(80) qy1(120) qy1(0)')")
      WRITE(112,
     &     "('lambda qy1(n0) qy2(n0) qy3(n0) qy1(0) qy2(0) qy3(0)  x')")
      DO iw = 1, nw - 1
         DO i = 1, nz
...
c                x = qy1_0/qy1_n0 - 1.
              ...
              ENDIF
!             qy1 = MAX(0.,MIN(1.,
!    &              qy1_n0 * (1. + x) / (1. + x * airden(i)/2.55E19)))
              qy1=1/(yg5(iw)+yg4(iw)*airden(i)/2.55E19)
              IF(wl(iw).gt.255. .and. wl(iw).lt.333. .and. i==1) THEN
                WRITE(111,"(F6.2,2(2X,F5.3))",advance='no')
     &             wl(iw),qy1_n0,qy1
                WRITE(112,"(F6.2,3(2X,F5.3))",advance='no')
     &             wl(iw),qy1_n0,qy2,qy3
               ELSEIF(wl(iw).gt.255. .and. wl(iw).lt.333.
     &               .and. (i==nz/3 .or. i==2*nz/3)) THEN
                WRITE(111,"(2X,F5.3)",advance='no') qy1
               ELSEIF(wl(iw).gt.255. .and. wl(iw).lt.333. .and. i==nz)
     &         THEN
                WRITE(111,"(2(2X,F5.3))")
     &             qy1,qy1_0
                WRITE(112,"(3(2X,F5.3),2X,F7.3)")
     &             qy1_0,qy2,qy3,x
              ENDIF
!             qy1 = MAX(0.,MIN(1.,qy1 * (1. + yg4(iw))/(1. + yg4(iw)
!    &              *airden(i)/2.465E19)))
!              qy1=1/(yg5(iw)+yg4(iw)*airden(i)/2.465E19)
             ELSEIF(mpress==2) THEN
              qy1 = yg1(iw)
              qy2 = yg2(iw)
              qy3 = yg3(iw)
...
*=============================================================================*
      ...
      SUBROUTINE r108(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! CH2(OH)CH2(ONO2) + hv
...
*=============================================================================*
      ...
      SUBROUTINE r109(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! CH3COCH2(ONO2) + hv
...
*=============================================================================*
      ...
      SUBROUTINE r110(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! C(CH3)3(ONO2) + hv
...
* 4:  IUPAC 2013 recommendation: T-dep. low res data between 251.7 and 350nm,
*                                high res. data between 226 - 251.7nm and 350 - 375nm
*     (MCM standard)
...
* 2:  IUPAC recommendation (MCM standard)
     ...
     &       ' HCHO cross section from IUPAC T-dep. low res. data.'
       ELSEIF(mabs.EQ.4) THEN
        WRITE(kout,'(A)')
     &  ' HCHO cross section from IUPAC mixed low and high res. data.'
       ELSE
        STOP "'mabs' not defined for HNO4 photolysis."
      ENDIF
...
* read IUPAC 2013 high res. data
     ...
     $        ,STATUS='old')
         ...
         CALL inter2(nw,wl,yg,n,x1,y298,ierr)
         IF (ierr .NE. 0) THEN
            WRITE(*,*) ierr, jlabel(j-1)
            STOP
         ENDIF
...
* yg4 = quantum yields for molecular channel, H2 + CO.
      ...
      IF(vers==1) OPEN(111,FILE='HCHOtuv.dat')
      IF(vers==2) OPEN(111,FILE='HCHOmcm.dat')
      WRITE(111,"(A,F5.1,A,1pE8.2,A)")
     &     '# T = ', tlev(2), 'K; p = ', airden(2), ' mlc/cm3'
      DO iz = 1, nz

         DO iw = 1, nw - 1

* correct cross section for temperature dependence:
           IF(mabs==2 .OR. mabs==4) sig = yg(iw)
...
* IUPAC quantum yield data
     ...
     &                       0.26,1.12,0.39,2.47,fact)
               fact = fact * 1e-19
               qymt = 1. / (1. + fact*airden(iz))
           ENDIF
         ENDIF

         sq(j-1,iz,iw) = sig * qyr300
         sq(j  ,iz,iw) = sig * qymt
         IF(iz==2) WRITE(111,"(F7.2,2X,1pE9.3,2(2X,F5.3))")
     &                   wc(iw),sig,qyr300/10,qymt/10
...
*=============================================================================*
      ...
      SUBROUTINE r138(nw,wl,wc,nz,tlev,airden,j,sq,jlabel) ! CH3COOH + hv
