...
* combine:
      ...
      IF(vers==1) OPEN(111,FILE='../data/debug/CH3CHOtuv.dat')
      IF(vers==2) OPEN(111,FILE='../data/debug/CH3CHOmcm.dat')
...
* or on overall quantum yield based on IUPAC 2013
     ...
     &              *airden(i)/2.465E19)))
            ...
            sq(j-1,i,iw) = sig * qy2
            sq(j  ,i,iw) = sig * qy3
            IF(iz==2) WRITE(111,"(F7.2,2X,1pE9.3,3(2X,F5.3))")
     &                      wc(iw),sig,qy1/10,qy2/10,qy3/10
...
* combine:
      ...
      IF(vers==1) OPEN(111,FILE='../data/debug/C2H5CHOtuv.dat')
      IF(vers==2) OPEN(111,FILE='../data/debug/C2H5CHOmcm.dat')
      WRITE(111,"(A,F5.1,A,1pE8.2,A)")
     &     '# T = ', tlev(2), 'K; p = ', airden(2), ' mlc/cm3'
      DO iw = 1, nw - 1
         DO i = 1, nz
...
* use Ster-Volmer pressure dependence:
            ...
            qy1 = MIN(qy1,1.)
            sq(j,i,iw) = sig * qy1
            IF(iz==2) WRITE(111,"(F7.2,2X,1pE9.3,2X,F5.3)")
     &                      wc(iw),sig,qy1/10
         ENDDO
      ENDDO
      CLOSE(111)
...
* combine:
      ...
      IF(vers==1) OPEN(111,FILE='../data/debug/MGLYtuv.dat')
      IF(vers==2) OPEN(111,FILE='../data/debug/MGLYmcm.dat')
      WRITE(111,"(A,F5.1,A,1pE8.2,A)")
     &     '# T = ', tlev(2), 'K; p = ', airden(2), ' mlc/cm3'
      DO iw = 1, nw - 1
...
*       Res. Lett., 31, L06111, doi:10.1029/2003GL018793.
     ...
     &       ' CH3COCH3 cross section from NOAA 1998.'
       ELSEIF(mabs.EQ.4) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCH3 cross section from JPL-2011.'
       ELSEIF(mabs.EQ.5) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCH3 cross section from IUPAC 2013.'
       ELSE
        STOP "'mabs' not defined for HOCH2CHO photolysis."
      ENDIF
     ...
     &       ' CH3COCH3 quantum yields from McKeen et al. 1997.'
       ELSEIF(myld.EQ.4) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCH3O quantum yields from Blitz et al. 2004.'
       ELSEIF(myld.EQ.5) THEN
        WRITE(kout,'(A)')
     &       ' CH3COCH3 quantum yields from IUPAC 2013.'
       ELSE
        STOP "'myld' not defined for HOCH2CHO photolysis."
      ENDIF
...
* yg4 = quantum yields for molecular channel, H2 + CO.
      ...
      IF(vers==1) OPEN(111,FILE='../data/debug/HCHOtuv.dat')
      IF(vers==2) OPEN(111,FILE='../data/debug/HCHOmcm.dat')
      WRITE(111,"(A,F5.1,A,1pE8.2,A)")
     &     '# T = ', tlev(2), 'K; p = ', airden(2), ' mlc/cm3'
      DO iz = 1, nz
