...
*=============================================================================*
      ...
      SUBROUTINE intermat(Av,Bv,Amin,Amax,Bmin,Bmax,
     &                    vA0B0,vA0B1,vA1B0,vA1B1,val)

*-----------------------------------------------------------------------------*
*=  PURPOSE:                                                                 =*
*=  Find linearly interpolated matrix element Q(A,B) for matrix with know    =*
*=  corner elements for any given A and B.                                   =*
*=  Q(Amin,Bmin) ......... Q(Amax,Bmin)                                      =*
*=      .       .              .                                             =*
*=      .         .            .                                             =*
*=      .           .          .                                             =*
*=      .    Q(A,B)   .        .                                             =*
...
*=      .                 .    .                                             =*
*=      .                   .  .                                             =*
*=  Q(Amin,Bmax) ......... Q(Amax,Bmax)                                      =*
*-----------------------------------------------------------------------------*
*=  PARAMETERS:                                                              =*
*=  A     - REAL, first dependent variable of Q(A,B)                      (I)=*
*=  Amin  - REAL, minimum value in range of A                             (I)=*
*=  Amax  - REAL, maximum value in range of A                             (I)=*
*=  B     - REAL, second dependent variable of Q(A,B)                     (I)=*
*=  Bmin  - REAL, minimum value in range of B                             (I)=*
*=  Bmax  - REAL, maximum value in range of B                             (I)=*
*=  vA0B0 - REAL, first corner element of matrix Q(Amin,Bmin)             (I)=*
*=  vA0B1 - REAL, second corner element of matrix Q(Amin,Bmax)            (I)=*
*=  vA1B0 - REAL, third corner element of matrix Q(Amax,Bmin)             (I)=*
*=  vA1B1 - REAL, fourth corner element of matrix Q(Amax,Bmax)            (I)=*
*=  val   - REAL, value of interpolated matrix element Q(A,B)             (O)=*
*-----------------------------------------------------------------------------*

      IMPLICIT NONE

! I/O:
      REAL         :: Av,A,Amin,Amax    ! current value and range quantity A
      REAL         :: Bv,B,Bmin,Bmax    ! current value and range quantity B
      REAL         :: val,vA0B0,vA0B1,vA1B0,vA1B1
                    ! interpolated value (and valid ranges)

! internal:
      REAL         :: valA0, valA1   ! Minimum(B), maximum(B) for interpolated A value

      A = Av                        ! save original value
      B = Bv                        ! (subroutine should not change A/B in main programme)
      A = MAX(Amin, MIN(A,Amax))    ! Don't allow interpolation outside range
      B = MAX(Bmin, MIN(B,Bmax))    ! Use max/min instead

      valA0 = vA0B0+(vA1B0-vA0B0)/(Amax-Amin)*(A-Amin)   !val(A,min)
      valA1 = vA0B1+(vA1B1-vA0B1)/(Amax-Amin)*(A-Amin)   !val(A,max)

      val   = valA0+(valA1-valA0)/(Bmax-Bmin)*(B-Bmin)   !val(A,B)

      END SUBROUTINE intermat
