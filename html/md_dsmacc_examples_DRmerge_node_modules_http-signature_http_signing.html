...
<h2>Nonces</h2>
<p>Nonces are out of scope for this document simply because many service providers fail to implement them correctly, or do not adopt security specifications because of the infrastructure complexity. Given the <code>header</code> parameterization, a service provider is fully enabled to add nonce semantics into this scheme by using something like an <code>x-request-nonce</code> header, and ensuring it is signed with the <code><a class="el" href="classDate.html">Date</a></code> header.</p>
<h2>Clock Skew</h2>
<p>As the default scheme is to sign the <code><a class="el" href="classDate.html">Date</a></code> header, service providers SHOULD protect against logged replay attacks by enforcing a clock skew. The server SHOULD be synchronized with NTP, and the recommendation in this specification is to allow 300s of clock skew (in either direction).</p>
<h2>Required Headers to Sign</h2>
<p>It is out of scope for this document to dictate what headers a service provider will want to enforce, but service providers SHOULD at minimum include the <code><a class="el" href="classDate.html">Date</a></code> header.</p>
<h1>References</h1>
